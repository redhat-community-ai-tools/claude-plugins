...
Reference pipelines that already do this correctly:
...
- Tags configured on the RPA should reference `{{ oci_version }}`. This variable resolves from
  ...
  `build-helm-chart-oci-ta`, `{{ oci_version }}` will not populate correctly.
...
6. **Generated version doesn't match expectations, or `{{ oci_version }}` is empty in the RPA**
   → check the `helm-` prefixed Git tag exists and was pushed (Stage 3), and that the RPA
   references `{{ oci_version }}` correctly (Stage 4).
